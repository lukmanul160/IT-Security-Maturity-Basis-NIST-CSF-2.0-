...
function getSession(token) { if (!token) return null; const session = sessions.get(token); if (!session) return null; if (Date.now() - session.createdAt >= sessionLifetime) { sessions.delete(token); return null; } return session; }
function destroySession(token) { if (token) sessions.delete(token); }
function parseCookies(header = '') { const cookies = {}; for (const value of String(header).split(';')) { const separator = value.indexOf('='); if (separator < 1) continue; try { const key = decodeURIComponent(value.slice(0, separator).trim()); const cookieValue = decodeURIComponent(value.slice(separator + 1).trim()); if (key) cookies[key] = cookieValue; } catch (error) { continue; } } return cookies; }
