...
const scoreControl = (item, kind) => { const current = kind === 'policy' ? policyScoreFor(item.id) : scoreFor(item.id); return `<div class="score-buttons" aria-label="${kind} maturity score">${[1, 2, 3, 4].map(score => `<button type="button" class="score-button ${current === score ? 'selected' : ''}" data-assessment-score="${score}" data-score-type="${kind}" data-control="${item.id}">${score}</button>`).join('')}</div>`; };
...
async function uploadAttachment(input) {
  ...
  try {
    updateUploadProgress(key, 2, `Uploading ${files.length} file${files.length === 1 ? '' : 's'}...`); const attachments = state.attachments[key] || []; state.attachments[key] = attachments; const metadataList = await saveFilesToServer(functionName, kind, files); metadataList.forEach(metadata => { const existing = attachments.findIndex(attachment => attachment.name === metadata.name); if (existing >= 0) attachments[existing] = metadata; else attachments.push(metadata); });
    save(); updateUploadProgress(key, 100, `${files.length} file${files.length === 1 ? '' : 's'} uploaded`, 'success'); renderCsfTable(); renderUploadedFiles(); if ($('assessmentView').classList.contains('active-view')) renderControls();
  } catch (error) { save(); const duplicate = error.message.includes('already exists'); updateUploadProgress(key, 100, duplicate ? 'File name already exists' : 'Upload failed', 'fail'); $('saveState').textContent = duplicate ? 'Nama file sudah ada' : 'Upload gagal'; renderCsfTable(); renderUploadedFiles(); if ($('assessmentView').classList.contains('active-view')) renderControls(); }
  input.value = '';
}
...
async function openAttachment(key, index) { const attachment = (state.attachments[key] || [])[index]; if (attachment) window.open(apiFileUrl(attachment.path), '_blank', 'noopener'); }
async function downloadAttachment(key, index) { const attachment = (state.attachments[key] || [])[index]; if (attachment) { const link = document.createElement('a'); link.href = apiFileUrl(attachment.path); link.download = attachment.name; link.click(); } }
async function deleteAttachment(key, index) { const attachments = state.attachments[key] || []; const attachment = attachments[index]; if (!attachment || !confirm(`Hapus lampiran ${attachment.name}?`)) return; await fetch(apiFileUrl(attachment.path), { method: 'DELETE' }); attachments.splice(index, 1); state.attachments[key] = attachments; save(); renderCsfTable(); renderUploadedFiles(); }
...
async function reset() {
  ...
  try {
    const response = await fetch('/api/assessment/reset', { method: 'POST' }); if (!response.ok) throw new Error('Server reset gagal');
    state = { scores: {}, policyScores: {}, practiceScores: {}, notes: {}, attachments: {} }; attachmentHandles.clear(); renderOverview(); renderCsfTable(); renderUploadedFiles(); if ($('assessmentView').classList.contains('active-view')) renderControls();
    localStorage.removeItem(uploadFolderStorageKey); syncUploadFolderStatus('Folder: upload (Node.js)');
    $('saveState').textContent = 'Assessment dan semua file berhasil direset';
